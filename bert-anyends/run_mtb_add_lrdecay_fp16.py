...
def main():
    ...
    for i in range(ntasks):    
        ...
        if use_gpu:
            model = SequenceClassification(configuration[i]).cuda()
        else: 
            model = SequenceClassification(configuration[i])
            
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate)
        sub_models.append(model)        
        sub_optimizer.append(optimizer)
    ...
    bert_optimizer = torch.optim.AdamW(Bert_model.parameters(), lr=args.learning_rate)
    ...
    for i in range(1, iterations+1):
        ...
        for j in range(ntasks):
            ...
            if use_gpu:
                ...
                label=data['labels']
            with autocast():
                output_inter = Bert_model(input_ids=input_ids, attention_mask=attention_mask, return_dict=True) 
                loss_single = sub_models[j](input=output_inter, labels=label)[0]
