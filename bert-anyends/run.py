...
from data import GlueDataArgs, GlueDataSets, ComputeMetrics
...
epochs = 10
model_name="sst2-nmli-sstb-qnli-distilbert-frozen-mtb"
...
def main():
    ...
    train_iter = list()
    for i in range(ntasks):
        train_iter.append(datasets[i].iterator("train", batch_size_train[i]))
...
def evaluate_each(main_model, sub_model, dataset, bs, metrics, mm="validation_matched"):
    ...
    eval_iter = dataset.iterator("dev", bs, mm)
